...
private typealias Bands = DynamicModelLoader.MemoryBands

private func hardware(_ memoryGB: Double) -> HardwareProfile {
    HardwareProfile(
        chip: "test",
...
struct DynamicModelLoaderGemmaTests {
    ...
    func atLowBoundary() {
        ...
        let tag = DynamicModelLoader.shared.defaultFallbackModel(hardware: hardware(Bands.lowMemoryGB))
        #expect(tag == ModelDefaults.localOpenAIModelID)
    }

    @Test("Default Mac band (16 GB / 32 GB) — Gemma E4B")
    func defaultBand() {
        for memoryGB in [16.0, 24.0, 32.0, 47.0] as [Double] {
            let tag = DynamicModelLoader.shared.defaultFallbackModel(hardware: hardware(memoryGB))
            #expect(tag == ModelDefaults.localOpenAIModelID, "\(memoryGB) GB should pick E4B")
        }
    }

    @Test("At high boundary (48 GB) — already 26B MoE, not E4B")
    func atHighBoundary() {
        let tag = DynamicModelLoader.shared.defaultFallbackModel(hardware: hardware(Bands.highMemoryGB))
        #expect(tag == "gemma4:26b")
    }

    @Test("Workstation band (48–95 GB) — Gemma 26B MoE")
    func workstationBand() {
        for memoryGB in [48.0, 64.0, 95.0] as [Double] {
            let tag = DynamicModelLoader.shared.defaultFallbackModel(hardware: hardware(memoryGB))
            #expect(tag == "gemma4:26b", "\(memoryGB) GB should pick 26b")
        }
    }

    @Test("At workstation boundary (96 GB) — already 31B, not 26B")
    func atWorkstationBoundary() {
        let tag = DynamicModelLoader.shared.defaultFallbackModel(hardware: hardware(Bands.workstationMemoryGB))
        #expect(tag == "gemma4:31b")
    }

    @Test("Mac Pro band (>=96 GB) — Gemma 31B dense")
    func macProBand() {
        for memoryGB in [96.0, 128.0, 192.0, 512.0] as [Double] {
            let tag = DynamicModelLoader.shared.defaultFallbackModel(hardware: hardware(memoryGB))
            #expect(tag == "gemma4:31b", "\(memoryGB) GB should pick 31b")
        }
    }
}
...
struct DynamicModelLoaderQwenTests {
    ...
    func atLowBoundary() {
        let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(Bands.lowMemoryGB))
        #expect(tag == "qwen3.5:9b")
    }

    @Test("Mid band (16 GB / 23 GB) — Qwen 3.5 9B")
    func midBand() {
        for memoryGB in [16.0, 20.0, 23.0] as [Double] {
            let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(memoryGB))
            #expect(tag == "qwen3.5:9b", "\(memoryGB) GB should pick 9b")
        }
    }

    @Test("At mid boundary (24 GB) — already 27B, not 9B")
    func atMidBoundary() {
        let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(Bands.midMemoryGB))
        #expect(tag == "qwen3.6:27b")
    }

    @Test("High band (24–47 GB) — Qwen 3.6 27B dense")
    func highBand() {
        for memoryGB in [24.0, 32.0, 47.0] as [Double] {
            let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(memoryGB))
            #expect(tag == "qwen3.6:27b", "\(memoryGB) GB should pick 27b")
        }
    }

    @Test("At high boundary (48 GB) — already 35B MoE, not 27B")
    func atHighBoundary() {
        let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(Bands.highMemoryGB))
    ...
    func workstationBand() {
        for memoryGB in [48.0, 64.0, 95.0] as [Double] {
            let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(memoryGB))
            #expect(tag == "qwen3.6:35b", "\(memoryGB) GB should pick 35b")
        }
    }

    @Test("At workstation boundary (96 GB) — already coder-next, not 35B")
    func atWorkstationBoundary() {
        let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(Bands.workstationMemoryGB))
        #expect(tag == "qwen3-coder-next")
    }

    @Test("Mac Pro band (>=96 GB) — qwen3-coder-next")
    func macProBand() {
        for memoryGB in [96.0, 192.0, 512.0] as [Double] {
            let tag = DynamicModelLoader.shared.recommendedQwenTag(hardware: hardware(memoryGB))
            #expect(tag == "qwen3-coder-next", "\(memoryGB) GB should pick coder-next")
        }
    }
}
...
struct DynamicModelLoaderMemoryBandsTests {
    ...
    func ascending() {
        let bands = [
            ...
            Bands.midMemoryGB,
            Bands.highMemoryGB,
            Bands.workstationMemoryGB
        ]
        for idx in 0..<(bands.count - 1) {
            #expect(bands[idx] < bands[idx + 1])
        }
    }
    ...
    func documentedValues() {
        ...
        #expect(Bands.lowMemoryGB == 12)
        #expect(Bands.midMemoryGB == 24)
        #expect(Bands.highMemoryGB == 48)
        #expect(Bands.workstationMemoryGB == 96)
    }
}
