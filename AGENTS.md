...
The pillars are intentionally Hermes-shaped but exploit Swoosh's structural advantages: every skill, goal, and manifestation is a durable record on the same audit/replay path as everything else; sync to the iPhone rides the same CloudKit channel as memories (per `Docs/iOS-Kernel-and-Sync.md`); platform constraints reuse the `ToolPlatform` infrastructure so the iPhone sees the runnable subset automatically.
...
- Apps live in three places: `App/` (menu-bar macOS target wired through XcodeGen), `Apps/SwooshMac` (SwiftPM-built standalone Mac shell) and `Apps/SwooshiOS` (the real iOS companion app — `SwooshiOSApp` + `RootView`/`ChatView`/`SettingsView`/`ClientSession`, wired through the XcodeGen `SwooshiOS` target), `Apps/SwooshDashboard` (reserved dashboard app directory). The iOS app deliberately imports only `SwooshClient` — never `SwooshKit` — so the daemon's `Process`-using deps don't break the build.
