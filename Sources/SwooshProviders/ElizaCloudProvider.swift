// SwooshProviders/ElizaCloudProvider.swift — 0.9P Experimental Eliza Cloud
//
...
// ═══════════════════════════════════════════════════════════════════
// MARK: - Eliza Cloud Provider
// ═══════════════════════════════════════════════════════════════════

public actor ElizaCloudProvider: ModelProviding {
    public nonisolated let providerID: ProviderID = "eliza-cloud"
    public nonisolated let displayName: String = "Eliza Cloud"
    ...
    public func health() async -> ProviderHealth {
        do {
            ...
            return ProviderHealth(providerID: providerID, status: .authMissing,
                                  message: "Set key: swoosh provider auth eliza-cloud --api-key")
        }
        ...
        do {
            ...
            return ProviderHealth(providerID: providerID, status: .unreachable,
                                  message: "Cannot reach Eliza Cloud: \(error.localizedDescription)")
        }
    }
    ...
    private func loadAPIKey() async throws -> String {
        do {
            return try await secrets.get(SecretRef("eliza-cloud", "api_key"))
        } catch {
            throw ProviderError.authMissing(providerID,
                "Eliza Cloud API key not found. Run: swoosh provider auth eliza-cloud --api-key")
        }
    }
