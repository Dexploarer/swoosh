// DetourNativeVoiceCatalog.swift — native macOS speech voice inventory (0.5A)

import AVFoundation
import Foundation

struct DetourNativeVoice: Identifiable, Equatable {
    let id: String
    let name: String
    let language: String
    let quality: Int
...
enum DetourNativeVoiceCatalog {
    static func voices(languagePrefix: String = "en") -> [DetourNativeVoice] {
        rankedVoices(
    ...
    static func defaultVoiceIdentifier() -> String? {
        ...
        return rankedVoices(englishVoices).first(where: { !$0.voiceTraits.contains(.isNoveltyVoice) })?.identifier
